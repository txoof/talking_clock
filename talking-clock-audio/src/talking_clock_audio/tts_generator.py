...
def build_vocab_map(config: dict[str, Any]) -> dict[str, str]:
    """
    Build the flat runtime vocabulary mapping for one locale.

    The returned mapping uses dotted symbolic keys such as:
    - 'words.midnight'
    - 'mode.standard'
    - 'number_words.12'

    and maps each key to the configured audio filename.
    ...
    -------
    dict[str, str]
        Flat symbolic token to audio filename mapping.
    """
...
def build_deduped_audio_text_map(config: dict[str, Any]) -> tuple[dict[str, str], dict[str, str]]:
    """
    Build a flat vocab map and a deduplicated filename to spoken text map.

    The locale YAML stores the spoken text for each vocab item, and this
    function derives the output filename for that same vocab item. Multiple symbolic vocab
    keys may point to the same audio filename. This function collapses those
    duplicates so each audio file is generated only once.
    ...
    tuple[dict[str, str], dict[str, str]]
        ...
        1. flat_vocab
           Symbolic token key to audio filename mapping

        2. audio_files
           Deduplicated audio filename to spoken text mapping
    """
    vocab = config.get('vocab', {})
    flat_vocab: dict[str, str] = {}
    ...
    for section_name, section_data in vocab.items():
        ...
        for key, value in section_data.items():
            ...
            if filename not in audio_files:
                audio_files[filename] = spoken_text

    return flat_vocab, audio_files
...
def generate_audio_package_with_tts(config: Dict[str, Any],
    ...
    Rules files are written separately by the rules compiler.
    ...
    vocab_map, audio_files = build_deduped_audio_text_map(config)
